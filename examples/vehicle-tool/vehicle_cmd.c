...
static void cmd_anki_vehicle_sdk_mode(int argcp, char **argvp)
{
        ...
        gatt_write_char(attrib, handle, value, plen,
                                        NULL, NULL);
       // g_timeout_add (1, cmd_anki_vehicle_set_speed, NULL);
}
...
// static gboolean speedfromfile(void)
// {

//         FILE *f;
//         char c;
        
//         f=fopen("setspeed.txt","r");
...
{
        // g_timeout_add (1, speedfromfile, NULL);
        uint8_t *value;
        size_t plen;
        int handle;
        uint16_t tempd;
        ...
        if(f != NULL)
	{
        ...
        anki_vehicle_msg_t msg;
        plen = anki_vehicle_msg_set_speed(&msg, tempd, accel);
        value = (uint8_t *)&msg;
        ...
        // while(TRUE)     
        // {
        //      cmd_anki_vehicle_set_speed(0, "");
        // }
	if(speed_readloop_on == 0) {
        	g_timeout_add (1000, cmd_anki_vehicle_set_speed, NULL);
		speed_readloop_on = 1;
	}
}
		...
		const char *dst_type, int psm)
{
	...
	event_loop = g_main_loop_new(NULL, FALSE);
        ...
        // g_timeout_add (1000, speedfromfile, NULL);
